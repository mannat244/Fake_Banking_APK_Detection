...
const About = () => (
  <section className="py-12 sm:py-16 px-4" 
    ...
    <div className="max-w-7xl mx-auto">
      <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-white mb-4 sm:mb-6 text-center">Service Overview</h2>
      <p className="text-center text-gray-300 mb-8 sm:mb-10 text-sm sm:text-base lg:text-lg px-4">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6 justify-items-center max-w-6xl mx-auto">
        {services.map((service, idx) => (
          <div key={idx} 
               className="rounded-2xl shadow-xl p-4 sm:p-6 w-full max-w-xs sm:max-w-sm flex flex-col items-center hover:scale-105 transition-all duration-300 group"
               style={{
                 background: "linear-gradient(145deg, rgba(16, 22, 36, 0.8) 0%, rgba(8, 15, 28, 0.9) 100%)",
                 border: "1px solid rgba(59, 130, 246, 0.2)",
                 backdropFilter: "blur(10px)"
               }}>
            <div className="mb-3 sm:mb-4 p-2 sm:p-3 rounded-lg border border-blue-400/30" 
                 style={{
                   background: "linear-gradient(145deg, rgba(59, 130, 246, 0.15) 0%, rgba(37, 99, 235, 0.25) 100%)"
                 }}>
              {service.icon}
            </div>
            <h3 className="text-white text-base sm:text-lg font-semibold mb-2 sm:mb-3 text-center leading-tight">{service.title}</h3>
            <p className="text-gray-300 text-center text-xs sm:text-sm leading-relaxed">{service.desc}</p>
          </div>
        ))}
      </div>
    </div>
